...
passport.deserializeUser(function(id, done) {
    ...
    UserCollection.findOne({ _id: id }, (error, user) => {
...
passport.use('admin-local', new LocalStrategy(
    function(username, password, done) {
        const UserCollection = DatabaseEngine.getUsersCollection();
        UserCollection.findOne({ username: username, isAdmin: "true" }, function (err, user) {
            if (err) return done(err)
            if (!user) return done(null, false, { message: 'Nome de utilizador incorreto.' })

            try {
                const result = comparePassword(user.password, password);
                if (!result) return done(null, false, { message: 'Password incorreta.' })
                return done(null, user)
            } catch (err) {
                return done(err);
            }
