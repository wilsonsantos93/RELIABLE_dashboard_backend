...
let mongo: any = {
  ...
  connectionString: `mongodb://${process.env.DB_USERNAME}:${process.env.DB_PASSWORD}@${process.env.DB_URL}/${process.env.DB_NAME}`,
  host: 'mongodb',
  port: '27017',
...
function getBinaryFileEnv(envVariable: string) {
  const fileVar = process.env[envVariable];
  return getFile(fileVar);
}

const meConfigMongodbServer: any = process.env.ME_CONFIG_MONGODB_SERVER
...
export default {
  ...
  site: {
    ...
    cookieKeyName: 'reliable-session',
  ...
  options: {
    ...
    logger: {skip: function(req: any, res:any) {return true;}},
