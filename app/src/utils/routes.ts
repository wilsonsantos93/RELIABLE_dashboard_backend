 ...
 */
function isAllowed(allowedRoles: string[], userRoles: string[]) {
 ...
 */
export function authenticate(...allowed_roles: string[] | null) {
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        if (req.isAuthenticated() && !req.headers['authorization']) {
            ...
            if (!allowed_roles.length || isAllowed(allowed_roles, user.roles)) return next()
            else return res.status(403).json({ error: 'Forbidden' })
        }
        // Authentication through Authorization Header allowing multiple strategies
        else return passport.authenticate(['admin-local', 'user-basic', 'api-jwt', 'api-key'], (error, user) => {
