 ...
 */
export async function requestWeather(locationCoordinates) {
    const apiURL = "https://api.weatherapi.com/v1/current.json?key=a1f415612c9543ea80a151844220103&q=" +
        locationCoordinates +
        "&aqi=yes";
 ...
 */
export function createBulkOps(data) {
    return data.map(d => {
        return { updateOne: {
                filter: {
 ...
 */
function extractDateFromString(str, regex = null) {
    if (!regex)
        regex = new RegExp("[0-9]{2,4}(-|/)[0-9]{2}(-|/)[0-9]{2,4}(( |_|T)[0-9]{0,2}(:|h|H||_)[0-9]{0,2}(:||_)[0-9]{0,2}(\.[0-9]{0,3})?){0,1}");
 ...
 */
function extractAndFormatDateFromString(str, regex = null) {
    ...
    if (time && (time.includes("h") || time.includes("H")))
        time = time.replace(new RegExp("h|H"), ":");
...
export async function transformData(data) {
    ...
    // Loop through data 
    for (const d of data) {
        if (!d.hasOwnProperty(WEATHER_FIELD_TO_MATCH))
            continue;
        const regionPossibleNames = allReplacements(d[WEATHER_FIELD_TO_MATCH], "-", " ");
        const projection = { _id: 1, geometry: 0 };
        const regions = await regionsCollection.find({ [DATABASE_FIELD_TO_MATCH]: { $in: regionPossibleNames } }, { projection })
        ...
        for (const key in d) {
            ...
            if (!originalDate)
                fieldsNotDate.push(key);
        }
        if (!originalDates.length)
            return [];
        // Insert dates into DB
        const bulkOps = originalDates.map((date) => {
        ...
        for (const date of originalDates) {
            ...
            const extractedDate = extractAndFormatDateFromString(date);
            const convertedDate = dayjs(extractedDate.date).tz(tz, true).toISOString();
            sample.weatherDateObjectId = (_a = datesFromDB.find(doc => new Date(doc.date).valueOf() == new Date(convertedDate).valueOf())) === null || _a === void 0 ? void 0 : _a._id;
            // Add region id and push to array
            for (const region of regions) {
                let newSample = Object.assign({}, sample);
                // Add region Id
                newSample.regionBorderFeatureObjectId = (region === null || region === void 0 ? void 0 : region._id) || null;
                // Push sample to array
                if (Object.keys(newSample).length > 0)
                    transformedData.push(newSample);
            }
        }
    }
    return transformedData;
}
export async function generateAlerts(locations, numDaysAhead) {
    var _a;
    try {
        if (!locations.length)
            return {};
        const weatherFields = await DatabaseEngine.getWeatherMetadataCollection().find({ active: true }).toArray();
        if (!weatherFields || !weatherFields.length)
            return {};
        const weatherField = weatherFields.find((f) => f.main == true && f.active == true);
        if (!weatherField)
            return {};
        let features = [];
        for (const location of locations) {
            ...
            const ix = features.findIndex((f) => f._id == stringId);
            if (ix < 0) {
        ...
        if (firstDate)
            startDate = new Date(firstDate.date);
        const { ALERT_NUM_DAYS_AHEAD, DB_REGION_NAME_FIELD } = await readGeneralMetadata();
        if (!numDaysAhead)
            numDaysAhead = parseInt(ALERT_NUM_DAYS_AHEAD);
        const endDate = new Date(new Date().valueOf() + (numDaysAhead * 24 * 60 * 60 * 1000));
        const weatherFieldAlertable = weatherField.ranges.filter((r) => r.alert == true);
        if (!weatherFieldAlertable || !weatherFieldAlertable.length)
            return {};
        const orQuery = [];
        for (const alertable of weatherFieldAlertable) {
            let min = alertable.min;
            let max = alertable.max;
            if (alertable.min == null || isNaN(alertable.min))
                min = -Infinity;
            if (alertable.max == null || isNaN(alertable.max))
                max = Infinity;
            orQuery.push({
        ...
        for (const alert of alerts) {
            ...
            for (const field of weatherFields) {
                ...
                for (let r of ranges) {
                    let min = r.min;
                    let max = r.max;
                    if (r.min == null || isNaN(r.min))
                        min = -Infinity;
                    if (r.max == null || isNaN(r.max))
                        max = Infinity;
            ...
            const label = (_a = [...weatherField.ranges].reverse().find((r) => (r.min <= value) && (value <= r.max))) === null || _a === void 0 ? void 0 : _a.label;
            alert.weather = {
...
export async function sendAlertsByEmail() {
    try {
        ...
        for (const user of users) {
            try {
                if (!user.locations || !user.locations.length || !user.email)
                    continue;
                const locations = user.locations.map((location) => JSON.parse(decrypt(location)))
                    .map((location) => {
                    return {
                        _id: location._id,
                        name: location.name,
...
function generateHtmlMessage(alerts, userId) {
    try {
        ...
        for (const a of alerts) {
            ...
            if (names.length)
                message += `<p><strong>${a.regionName} (${names.join()})</strong>`;
            else
                message += `<p><strong>${a.regionName}</strong>`;
            message += ` com ${a.weather.name} de `;
            if (a.weather.label)
                message += `<strong>${a.weather.value} (${a.weather.label})</strong>`;
            else
                message += `<strong>${a.weather.value}</strong>`;
