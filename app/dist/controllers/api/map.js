import { collectionExistsInDatabase, queryAllCoordinatesReferenceSystems, queryWeatherDocuments } from "../../utils/database.js";
 ...
 */
// Sends an array of geoJSONs with the border regions and its weather information on a certain date
export async function handleGetRegionBordersAndWeatherByDate(req, res) {
    // Check if date or dateId was sent 
    if (!req.query.dateId && !req.query.date)
        return res.status(500).json("Date is missing.");
    // Transform date id string into ObjectId
    let weatherDateObjectID;
    if (req.query.date) {
        req.query.date = sanitize(req.query.date);
        const datesCollection = DatabaseEngine.getWeatherDatesCollection();
        const dateId = await datesCollection.find({ "date": { $lte: new Date(req.query.date) } }).sort({ "date": -1 }).limit(1).toArray();
        if (!dateId.length)
    ...
    let weatherCollectionExists = await collectionExistsInDatabase(weatherCollectionName, DatabaseEngine.getDashboardDatabase());
    ...
    if (regionBordersCollectionExists && weatherCollectionExists) {
        ...
        for (const crs of crsQueryResults) {
            ...
            if (req.query.sw_lng && req.query.sw_lat && req.query.ne_lng && req.query.ne_lat) {
                coordinates = {
                    sw_lng: parseFloat(sanitize(req.query.sw_lng)),
                    sw_lat: parseFloat(sanitize(req.query.sw_lat)),
                    ne_lng: parseFloat(sanitize(req.query.ne_lng)),
                    ne_lat: parseFloat(sanitize(req.query.ne_lat))
                };
            }
            // Set flag useCenters
            ...
            for (const weatherDocument of weatherDocuments) {
                if (!weatherDocument.weather.length)
                    weatherDocument.weather = null;
                else
                    weatherDocument.weather = weatherDocument.weather[0].weather;
            }
            geoJSON.features = weatherDocuments;
