 ...
 */
export function getRegionsPage(req, res) {
 ...
 */
export async function getWeatherPage(req, res) {
    try {
 ...
 */
export async function handleGetRegions(req, res) {
    try {
        let projection = {};
 ...
 */
export async function handleDeleteRegions(req, res) {
    try {
 ...
 */
export async function handleGetRegionWithWeather(req, res) {
    var _a;
    try {
        ...
        if (req.query.columns) {
            const reqColumns = req.query.columns;
        ...
        if (columns && columns.length) {
            for (const col of columns) {
                if (!col.search.value || col.search.value == '' || col.name == "date")
                    continue;
        ...
        pipeline.push({ $skip: parseInt(req.query.start) || 0 });
 ...
 */
export async function handleGetRegionFields(req, res) {
    try {
 ...
 * @param res Client HTTP response object
 */
export async function handleDeleteRegion(req, res) {
    try {
 ...
 */
export async function handleDeleteWeatherRegion(req, res) {
    try {
        const id = req.params.id;
...
export async function handleCalculateCenters(req, res) {
    ...
    //* If the region borders collection exists, calculate and update the centers of each feature in the collection
        ...
        let featuresQueryResults = await queryAllFeatureDocuments(featuresQueryProjection);
        for (const feature of featuresQueryResults) {
            ...
            await DatabaseEngine.getFeaturesCollection().updateOne({ _id: feature._id }, // Updates the region feature document that has the same id as the current feature
            {
                $set: {
                    center: center,
                },
